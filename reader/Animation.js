...
class HopAnimation {
	constructor(span, from, to) {
		...
		this.step = {
			x: this.vector.x / span,
			y: Math.PI / span,
			z: this.vector.z / span
		};
	...
	update(dt) {
		if (!this.finished) {
			this.position.x += this.step.x * (dt / 1000);
			this.angle -= this.step.y * (dt / 1000);
			this.position.y = Math.sin(this.angle);
			this.position.z += this.step.z * (dt / 1000);
			this.elapsed += dt;
		}

		if (this.elapsed >= this.span && !this.finished) {
			console.log(`Finished in ${this.elapsed/1000} seconds.`);
			this.finished = true;
		}
    }
